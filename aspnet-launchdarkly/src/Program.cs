...
{
    var online = webApplicationBuilder.Configuration.GetValue<bool>("LaunchDarkly:Online");
    ...
    if (online)
        ...
        ldConfig = Configuration.Default(launchDarklySdkKey);
    else
        ldConfig = Configuration.Builder("TEST-SDK-KEY")
            .DataSource(
                FileData.DataSource()
                    .FilePaths("../ld-flagdata.json")
                    .AutoUpdate(true)
            )
